...
public class Population_BIN
{
    public static void main(String[]args) throws Exception
    {
        ...
        try
        {
            in = new BufferedReader(new FileReader("C:/MyJavaPrograms/Assignment6_ADV/PovertyData(1).txt"));
            ...
            try
            {
                // Write the summarized object array contents into the new binary output file
                out = new DataOutputStream(
                      new BufferedOutputStream(
                      new FileOutputStream("C:/MyJavaPrograms/Assignment6_ADV/Out_PovertyData.dat")));
                String header = String.format("%10s %10s %10s %10s %10s \n", "State", "Population", "Child Population", "Child Poverty Population", "% Child Poverty");
                String dashes = String.format("%10s %10s %10s %10s %10s \n", "-----", "----------", "----------------", "------------------------", "---------------");
                out.writeUTF(header);
                out.writeUTF(dashes);
                ...
                {
                    // System.out.println((double)(stateInfo[i].getChildPovPop())/((double)(stateInfo[i].getChildPop())));
                    double childPovPercent = ((double)(stateInfo[i].getChildPovPop())/(double)(stateInfo[i].getChildPop()))*100.0;
                    String line = String.format("%10s %10d %16d %24d %15.2f \n", stateInfo[i].getStateCode(), stateInfo[i].getTotalPop(), stateInfo[i].getChildPop(), stateInfo[i].getChildPovPop(), childPovPercent);
                    out.writeUTF(line);
